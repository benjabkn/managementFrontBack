      ...
      <table class="reservation-table">
        ...
        <tbody>
          <tr v-for="(reservation, index) in reservations" :key="reservation._id">
            ...
            <td>{{ reservation.zone.description }}</td>
            <td>Mesa {{ reservation.tableId.tableNumber }} Cantidad de personas:{{ reservation.tableId.ability }}</td>
    ...
    methods: {
      ...
      updateEndDate() {
        if (this.reservation.startDate && this.reservation.startTime) {
          ...
          const fecha = dayjs(endDateTime);
          const fechaFormateada = fecha.format("HH:mm");

          // Actualizar la fecha de fin
          this.reservation.endDate = fechaFormateada
        }
      },
  
      // Función para formatear la fecha de fin (en caso de que queramos mostrarla correctamente en un input)
      formatDateForDisplay(date) {
        if (!date) return '';
        const [datePart, timePart] = date.split('T');
        return `${datePart} ${timePart.slice(0, 5)}`; // Ejemplo: '2024-12-04 14:00'
      },
      ...
      async makeReservation() {
        try {
          const reservationData = {
            ...
            endDate: new Date(this.reservation.endDate).toISOString(),
